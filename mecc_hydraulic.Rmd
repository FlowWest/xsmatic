...
Future improvement to script should include functions to avoid duplication:

# Interpolate water surface for selected discharge - Q100
...
xs_2007 %>% arrange(sta) %>%
      ...
      geom_line(aes(y = gse)) + 
      geom_line(aes(y = wse)) + 
      ylab("Station (ft)") + xlab("Elevation (ft)") + ggtitle("2007 Baseline") + coord_fixed(ratio = 1)
```
```{r message=FALSE, warning=FALSE}
xs_2023 %>% arrange(sta) %>%
      ...
      geom_line(aes(y = gse)) + 
      geom_line(aes(y = wse)) + 
      ylab("Station (ft)") + xlab("Elevation (ft)") + ggtitle("2023 Proposed") + coord_fixed(ratio = 1)
```
...
ggplot() + 
  ...
  geom_line(data = xs_2023, aes(x = sta, y = case_when(wse_2023 > gse ~ wse_2023), 
      ...
      ylab("Station (ft)") + xlab("Elevation (ft)") + theme(legend.position="top", legend.title=element_blank()) + coord_fixed(ratio = 1)
```
...
# Interpolate water surface for selected discharge - Q2
